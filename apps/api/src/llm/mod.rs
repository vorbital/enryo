...
pub async fn score_pertinence(content: &str, llm_url: &str) -> Result<bool, Box<dyn std::error::Error + Send + Sync>> {
    ...
    if let Ok(resp) = response {
        if resp.status().is_success() {
            let embedding: OllamaEmbeddingResponse = resp.json().await?;
...
fn calculate_pertinence_heuristic(content: &str) -> bool {
    ...
    let off_topic_patterns = [
        "meme", "lol", "lmao", "haha", "funny", 
        "off topic", "random", "unrelated",
    ...
    let on_topic_patterns = [
        "code", "bug", "error", "fix", "pr",
        "review", "merge", "deploy", "test",
        "feature", "issue", "ticket", "task",
        "meeting", "deadline", "sprint",
    ];
...
pub async fn get_embedding(content: &str, llm_url: &str) -> Result<Vec<f32>, Box<dyn std::error::Error + Send + Sync>> {
    let client = reqwest::Client::new();
